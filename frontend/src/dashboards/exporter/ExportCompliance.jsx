...
const STATUS_CFG = {
  "VERIFIED":     { color: "#15803d", bg: "rgba(34,197,94,0.08)"   },
  "ACTIVE":       { color: "#0369a1", bg: "rgba(56,189,248,0.08)"  },
  "RENEWAL DUE":  { color: "#b45309", bg: "rgba(251,191,36,0.08)"  },
  "EXPIRED":      { color: "#dc2626", bg: "rgba(239,68,68,0.08)"   },
  "PENDING":      { color: "#7c3aed", bg: "rgba(167,139,250,0.08)" },
};

const DOC_ICONS = {
  "Phytosanitary Certificate":   Sprout,
  "Certificate of Origin":       FileCheck,
  "APEDA Registration (RCMC)":   Landmark,
  "FSSAI Export License":        ShieldCheck,
  "IEC Code (Import Export Code)":FileText,
  "GlobalGAP Certification":     Globe,
  "Fumigation Certificate":      Wind,
  "Inspection Certificate":      Search,
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 20, fontWeight: 800, color: "#0f172a", display: "flex", alignItems: "center", gap: 8 }}>
              <FileText size={22} color="#d97706" /> Add Compliance Document
            </div>
...
<body>
  ...
  <table>
    <tr><th>Document Parameter</th><th>Details</th></tr>
    <tr><td>Document Category</td><td>${doc.docType}</td></tr>
    <tr><td>Reference / Registration No.</td><td><strong>${doc.refNumber || "N/A"}</strong></td></tr>
    <tr><td>Issuing Authority</td><td>${doc.authority || "N/A"}</td></tr>
    <tr><td>Authorized Exporter</td><td>${user?.name || "Registered Exporter"}</td></tr>
    <tr><td>Validity Period</td><td>${validity}</td></tr>
    <tr><td>Effective Date</td><td>${fmtDate(doc.validFrom) || "Recorded on file"}</td></tr>
    <tr><td>System Verification</td><td>Verified Active via AgroConnect 360 Vault</td></tr>
  </table>
  ${doc.notes ? `<div style="margin:20px 0;padding:12px;background:#f8fafc;border-left:3px solid #b45309;font-size:12px;"><strong>Notes:</strong> ${doc.notes}</div>` : ""}
  <div class="ftr">
    <div class="seal">AGROCONNECT 360 · DIGITALLY RECORDED</div>
    <div>System Generated on ${new Date().toLocaleString("en-IN")}</div>
  </div>
    ...
    <>
      ...
      <div className="pg-head">
        <div>
          ...
          <h1 className="pg-title" style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <FileText size={26} color="#d97706" /> Export Customs & Certification Hub
          </h1>
      ...
      {(renewalDue > 0 || expired > 0) && !loading && (
        <div style={{ marginBottom: 20, display: "flex", gap: 10, flexWrap: "wrap" }}>
          {renewalDue > 0 && (
            <div style={{ flex: 1, minWidth: 200, padding: "12px 16px", borderRadius: 12, background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.25)", color: "#b45309", fontWeight: 700, fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}>
              <AlertTriangle size={16} color="#d97706" style={{ flexShrink: 0 }} />
              <span>{renewalDue} document{renewalDue > 1 ? "s" : ""} due for renewal</span>
            </div>
          )}
          {expired > 0 && (
            <div style={{ flex: 1, minWidth: 200, padding: "12px 16px", borderRadius: 12, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", color: "#dc2626", fontWeight: 700, fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}>
              <AlertCircle size={16} color="#dc2626" style={{ flexShrink: 0 }} />
              <span>{expired} document{expired > 1 ? "s" : ""} expired — update immediately</span>
            </div>
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "72px 24px" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 14 }}>
            <FileText size={52} color="#d97706" strokeWidth={1.5} />
          </div>
          <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 22, fontWeight: 800, color: "#0f172a", marginBottom: 8 }}>No Documents Yet</div>
          ...
          <button className="btn-gold" onClick={() => setShowAdd(true)} style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
            <FileText size={16} /> Add First Document
          </button>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 18 }}>
          {docs.map(doc => {
            ...
            const IconComponent = DOC_ICONS[doc.docType] || FileText;
            ...
            return (
              <div key={doc._id} className="card" style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", gap: 12 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <span style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 42, height: 42, borderRadius: 10, background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.18)" }}>
                    <IconComponent size={22} color="#d97706" />
                  </span>
                ...
                {/* Title + authority */}
                <div>
                  <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 16, fontWeight: 800, color: "#0f172a", marginBottom: 4 }}>{doc.title}</div>
                  {doc.authority && <div style={{ fontSize: 12, color: "#a38a5d" }}>Issuing Body: <strong style={{ color: "#0f172a" }}>{doc.authority}</strong></div>}
                  {doc.refNumber && <div style={{ fontSize: 11, color: "#a38a5d", marginTop: 2 }}>Ref: {doc.refNumber}</div>}
                </div>

                {/* Validity pill */}
                <div style={{ fontSize: 11, background: isExpired ? "rgba(239,68,68,0.1)" : isExpiring ? "rgba(251,191,36,0.1)" : "rgba(0,0,0,0.2)", padding: "8px 12px", borderRadius: 10, color: isExpired ? "#f87171" : isExpiring ? "#fbbf24" : "#a38a5d", fontWeight: isExpiring || isExpired ? 700 : 400, display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button className="btn-ghost" style={{ flex: 1, justifyContent: "center", fontSize: 11, display: "inline-flex", alignItems: "center", gap: 6 }} onClick={() => printDocPdf(doc, user)}>
                    <Printer size={13} /> Print / PDF
                  </button>

                  {deleteConfId === doc._id ? (
                    <>
                      <button style={{ flex: 1, padding: "8px", borderRadius: 10, background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.25)", color: "#dc2626", fontWeight: 700, fontSize: 11, cursor: "pointer" }} onClick={() => deleteDoc(doc._id)}>
                        Yes, Delete
                      </button>
                      <button style={{ flex: 1, padding: "8px", borderRadius: 10, background: "#f8fafc", border: "1px solid #e2e8f0", color: "#a38a5d", fontWeight: 600, fontSize: 11, cursor: "pointer" }} onClick={() => setDeleteConfId(null)}>
                        Cancel
                      </button>
                    </>
                  ) : (
                    <button className="btn-ghost" style={{ flex: 1, justifyContent: "center", fontSize: 11, color: "#dc2626", borderColor: "rgba(239,68,68,0.2)", display: "inline-flex", alignItems: "center", gap: 6 }} onClick={() => setDeleteConfId(doc._id)}>
